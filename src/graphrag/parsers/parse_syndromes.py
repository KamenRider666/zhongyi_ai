"""解析 中医临床诊疗术语证候 → syndromes.jsonl

用法: uv run python src/graphrag/parsers/parse_syndromes.py --test
   或: uv run python src/graphrag/parsers/parse_syndromes.py
"""
...
DATA_FILE = "/src/graphrag/data/中医临床诊疗术语证候.txt"
OUTPUT_FILE = "/src/graphrag/data/syndromes.jsonl"
...
# 分析: 一个证候条目约 3~20 行, 每行 ~60 字符, 最大约 1200 字符
MAX_INSTANCE_CHARS = 1200
CHUNK_SIZE = int(MAX_INSTANCE_CHARS * 2.2)  # 2640
...
SYNDROME_TEMPLATE = {
    ...
    "is_category": False,  # 是否只是类目词（如"泛指...一类证候"）
    "parent_code": "父级编号（如 3.3.1 → 3.3），若无则 null",
}
...
def parse_first_syndrome(chunk: str) -> tuple[dict | None, int]:
    """用 LLM 从 chunk 中提取第一个证候条目"""
    schema_str = json.dumps(SYNDROME_TEMPLATE, ensure_ascii=False, indent=2)

    system = (
        "你是一个精确的中医证候术语文本解析器。从一段包含多个证候条目的文本中，"
        "精确找到「第一个完整的证候条目」，提取为指定 JSON 格式。\n\n"
        "识别规则：\n"
        "1. 每个条目以编号开头（如 \"3.1\\n\"），然后是一行或多行名称，然后是定义文本。\n"
        "2. 如果名称后面有 \"注：...\" 则是注解，应包含在 definition 中。\n"
        "3. 有些是纯类目词（如\"泛指...一类证候\"），标记 is_category=true。\n"
        "4. 别名在名称行之后、定义行之前提取。\n"
        "5. 层级关系从编号推导：3.3.1 的 parent 是 3.3, 3.1 的 parent 是 null。\n\n"
        "输出格式：\n"
...
def _run(max_rounds: int | None = None):
    ...
    output_file = OUTPUT_FILE if max_rounds is None else (
        SRC_DIR.parent / "data" / "syndromes_test.jsonl"
    )
    ...
    with open(output_file, "w", encoding="utf-8") as f_out:
        while text.strip():
            ...
            parsed, cutoff = parse_first_syndrome(chunk)
    ...
    print(f"\n{'=' * 60}")
    print(f"完成! 共解析 {count} 条证候, 输出到 {output_file}")
